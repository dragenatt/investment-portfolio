...
export const PROCEDENCIA_ETIQUETAS: Record<Procedencia, string> = {
  ...
  'no-aplica': 'No entra en este calculo',
}
...
const ENSENANZAS: Record<ConceptoId, Ensenanza> = {
  'rendimiento-esperado': {
    ...
    porQueImporta:
      'Es la palanca mas fuerte de toda la proyeccion. Mover un solo punto porcentual cambia el resultado a largo plazo mucho mas que subir la aportacion, y por eso importa de donde sale la cifra.',
    errorComun:
  ...
  volatilidad: {
    ...
    termino: 'Volatilidad',
    definicion:
      'Cuanto se aparta el rendimiento real de su promedio, medida como desviacion estandar anual. Describe el tamaño tipico del vaiven, no su direccion.',
    porQueImporta:
      'Dos planes con el mismo rendimiento esperado y distinta volatilidad no son el mismo plan: el mas volatil tiene una cola de resultados malos mucho mas larga, y esa cola decide si el plan se aguanta.',
    errorComun:
      'Confundirla con perdida. Volatilidad alta significa movimientos grandes en las dos direcciones; lo que la vuelve peligrosa es que empuja a vender en el peor momento.',
  },
  'interes-compuesto': {
    ...
    porQueImporta:
      'Es la razon por la que el plazo pesa mas que el monto. Los ultimos tramos de un horizonte largo aportan mucho mas valor que los primeros, aunque la aportacion mensual sea la misma.',
    errorComun:
      'Suponer que el efecto es lineal, y que solo funciona hacia arriba. Una caida tambien compone, y recuperarla exige un porcentaje mayor que el que se perdio.',
  },
  'monte-carlo': {
    id: 'monte-carlo',
    termino: 'Simulacion de Monte Carlo',
    definicion:
      'En lugar de proyectar un unico camino, el modelo sortea miles de secuencias de rendimientos y observa como termina cada una. El resultado es un abanico de finales posibles, no una linea.',
    porQueImporta:
      'Un solo camino esconde el rango. El abanico muestra cuanto puede separarse el mejor caso del peor bajo los mismos supuestos, que es la informacion que hace falta para decidir.',
    errorComun:
  ...
  percentiles: {
    ...
    errorComun:
      'Tratar el corte bajo como el peor caso posible. Es solo el limite de la zona desfavorable que se simulo; por debajo queda una fraccion de trayectorias todavia peores.',
  },
  diversificacion: {
    id: 'diversificacion',
    ...
    porQueImporta:
      'Es la unica forma conocida de bajar el riesgo sin renunciar en la misma proporcion al rendimiento, y depende de como se relacionan entre si los activos, no de cuantos hay.',
    errorComun:
      'Contar posiciones. Varios fondos del mismo indice son una sola apuesta repetida; dos activos que suelen moverse al reves diversifican mucho mas que diez que se mueven juntos.',
  },
  horizonte: {
    id: 'horizonte',
    termino: 'Horizonte',
    ...
    errorComun:
      'Dar por hecho que un plazo largo elimina el riesgo. Reduce la probabilidad de terminar en perdida, pero agranda la desviacion posible medida en dinero.',
  },
  riesgo: {
    id: 'riesgo',
    termino: 'Riesgo',
    ...
    errorComun:
      'Medirlo por lo que se siente en un mes malo. El riesgo que cuenta es el de no llegar, y a veces la decision mas arriesgada resulta ser quedarse fuera.',
  },
  'tasa-libre': {
    id: 'tasa-libre',
    termino: 'Tasa libre de riesgo',
    definicion:
      'El rendimiento que se obtiene sin asumir riesgo de mercado, tipicamente deuda publica de corto plazo. Es la referencia contra la que se juzga cualquier otro rendimiento.',
    porQueImporta:
      'Solo la parte que supera esa referencia es paga por asumir riesgo. En esta app la tasa libre entra en las metricas de riesgo de tu cartera, como el Sharpe y el Sortino, no en este proyector.',
    errorComun:
  ...
  aportacion: {
    ...
    termino: 'Aportacion',
    definicion:
      'El dinero nuevo que entra cada periodo. Es la unica variable del plan que se controla por completo, a diferencia del rendimiento.',
    porQueImporta:
      'En los primeros tramos del plan aporta mas al saldo que el rendimiento, y no depende de que el mercado coopere. Es tambien la palanca que primero se rompe si no se puede sostener.',
    errorComun:
      'Subirla hasta que la proyeccion muestre la cifra deseada. La aportacion que hace cuadrar el numero solo sirve si se puede pagar todos los meses del horizonte.',
  },
}
...
export function explicacionEducativa(
  ...
  return {
    ...
    aviso:
      'Esta seccion explica los conceptos que usa el proyector. Es material educativo: no cambia ninguna cifra de las de arriba, y ninguna de estas definiciones es una recomendacion de inversion.',
  }
}
...
function describirTasaLibre(
  ...
  if (!tasaLibre || !Number.isFinite(tasaLibre.anualPct)) {
    return {
      valor: 'No entra en esta proyeccion: el modelo compone un solo rendimiento supuesto',
      procedencia: 'no-aplica',
    }
  }
  return {
