...
const NO_VALUE = 'Todavía no hay datos suficientes para calcular esta métrica.'
const EMPTY = '—'
...
const EXAMPLES: Record<MetricId, WorkedExample> = {
  return: example(
    'Inviertes 10,000 y un año después valen 11,200.',
  ...
  volatility: example(
    ...
    `Desviación estándar muestral de los diez = ${fmt((calculateVolatility(EXAMPLE_DAILY) / Math.sqrt(TRADING_DAYS_PER_YEAR)) * 100)}% diario; x raíz(${TRADING_DAYS_PER_YEAR}) para anualizar`,
    calculateVolatility(EXAMPLE_DAILY) * 100,
    'percent',
    (v) => `La volatilidad anualizada es ${v}.`,
  ),
  sharpe: example(
    'Un portafolio rinde 10% al año con 15% de volatilidad; la tasa libre de riesgo es 4%.',
    '(10 - 4) / 15',
    EXAMPLE_SHARPE,
    'ratio',
  ...
  alpha: example(
    'Un portafolio con beta 1.2 rinde 14% en un año en que el índice rinde 10%.',
    '14 - 1.2 x 10 = 14 - 12',
    14 - 1.2 * 10,
    'ratio',
  ...
  var: example(
    ...
    'percent',
    (v) => `El VaR al 95% es ${v}: en 19 de cada 20 días la pérdida no superó esa cifra.`,
  ),
  cvar: example(
    ...
    'percent',
    (v) => `El CVaR al 95% es ${v}: lo que en promedio se perdió los días que sí rompieron el umbral.`,
  ),
  maxDrawdown: example(
  ...
  trackingError: example(
    ...
    `${EXAMPLE_TE_DAILY_PCT} x raíz(${TRADING_DAYS_PER_YEAR})`,
    EXAMPLE_TE_DAILY_PCT * Math.sqrt(TRADING_DAYS_PER_YEAR),
    'percent',
    (v) => `El tracking error es ${v}: en un año típico el portafolio se separa del índice más o menos esa cantidad.`,
  ),
  informationRatio: example(
    ...
    'ratio',
    (v) => `El information ratio es ${v}: cada punto de separación del índice pagó esa fracción de punto de alpha.`,
  ),
  hhi: example(
    ...
    EXAMPLE_HHI,
    'hhi',
    (v) => `El HHI es ${v}, equivalente a ${fmt(1 / EXAMPLE_HHI, 1)} posiciones del mismo tamaño aunque haya 3.`,
  ),
  effectiveBets: example(
    'Tres activos con la misma volatilidad: dos con correlación 0.9 entre sí y un tercero independiente.',
  ...
  xirr: example(
    ...
    'percent',
    (v) => `El XIRR es ${v} anual. Ganaste 1,200 sobre 15,000, pero 5,000 solo estuvieron medio año invertidos.`,
  ),
  twr: example(
    'Un primer periodo rinde +10% y el segundo -5%, sin importar cuánto dinero entró en cada uno.',
    '(1.10 x 0.95) - 1',
    (1.1 * 0.95 - 1) * 100,
    'percent',
    (v) => `El TWR es ${v}.`,
...
const DEFINITIONS: Record<MetricId, Definition> = {
  return: {
    ...
    definition:
      'Cuánto creció o cayó el valor de la inversión en el periodo, expresado como porcentaje de lo que valía al principio.',
    source: null,
    read: (v) =>
      v >= 0
  ...
  volatility: {
    name: 'Volatilidad',
    unit: 'percent',
    formula: () => 'sigma = desviación estándar de los rendimientos x raíz(periodos por año)',
    definition:
      'Qué tanto se mueve el valor de la inversión, hacia arriba y hacia abajo. No mide pérdidas: mide inestabilidad, y una subida brusca cuenta igual que una caída.',
    source: null,
    read: (v, ctx) => {
      const { adjective, q } = cadence(ctx.periodsPerYear)
      return (
        `Tu ${fmt(v)}% anual sale de rendimientos ${adjective} (x raíz(${q})). ` +
        `Si los rendimientos fueran normales, en unos 2 de cada 3 años el resultado quedaría a menos de ${fmt(v)} puntos del promedio; en la práctica las colas son más gordas y los años extremos llegan más seguido de lo que eso sugiere.`
      )
    },
  },
  ...
  sharpe: {
    ...
    definition:
      'Cuánto rendimiento obtienes por encima de la tasa libre de riesgo por cada unidad de volatilidad que corres.',
    ...
    read: (v, ctx) => {
      ...
      const literal =
        v < 0
          ? `Tu ${fmt(v)} es negativo${rf}: el portafolio rindió menos que la tasa libre de riesgo, así que el riesgo que corriste no se pagó.`
      ...
      if (!q || !T || q <= 0 || T < 2) {
        return `${literal} Sin saber cuánto historial lo respalda no se puede decir qué tan preciso es.`
      }
      const se = Math.sqrt((q + (v * v) / 2) / T)
      const years = T / q
      return (
        `${literal} Con ${T} observaciones (${fmt(years, 1)} años), su error estándar es ${fmt(se)} (Lo, 2002): ` +
        `el intervalo de 95% va de ${fmt(v - 1.96 * se)} a ${fmt(v + 1.96 * se)}. ` +
        (v - 1.96 * se <= 0 && v + 1.96 * se >= 0
          ? 'Ese intervalo incluye el cero, así que con este historial no se distingue de un portafolio sin ninguna ventaja sobre la tasa libre.'
  ...
  sortino: {
    name: 'Sortino',
    unit: 'ratio',
    formula: () => 'Sortino = (Rp - Rf) / sigma_bajista, con sigma_bajista = raíz(promedio de min(0, r - objetivo)²)',
    definition:
    ...
    read: (v, ctx) => {
      if (v < 0) {
        return `Tu ${fmt(v)} negativo indica que el portafolio no superó la tasa libre de riesgo aun contando solo las caídas.`
      }
      const sharpe = ctx.sharpe
      if (sharpe === undefined || !Number.isFinite(sharpe) || sharpe <= 0) {
        return `Tu ${fmt(v)} mide el rendimiento por unidad de riesgo a la baja. Si es notablemente mayor que tu Sharpe, tus movimientos bruscos han sido más al alza que a la baja.`
      }
      ...
      return (
        ...
        (ratio > 1
          ...
          : 'tus caídas pesan tanto como toda tu volatilidad.')
      )
    },
  },
  ...
  beta: {
    ...
    definition:
      'Cuánto se mueve tu portafolio cuando se mueve el índice de referencia. Es sensibilidad, no calidad: una beta alta no es mejor ni peor, es más movimiento.',
    source: null,
    read: (v, ctx) => {
      const bench = ctx.benchmarkName ?? 'el benchmark'
      if (v > 1.1) {
        return `Tu ${fmt(v)} amplifica ${bench}: cuando sube 1%, tu portafolio ha tendido a subir cerca de ${fmt(v)}% — y cuando baja, también baja más.`
      }
      if (v < 0.9) {
  ...
  var: {
    ...
    unit: 'percent',
    formula: () => 'VaR_95 = pérdida en el percentil 5 de los rendimientos históricos',
    definition:
      'La pérdida que no se superó en 95 de cada 100 periodos observados. Es un umbral, no un techo: no dice nada sobre qué tan mal fue el 5% restante.',
    source: null,
    read: (v, ctx) => {
      const { bars } = cadence(ctx.periodsPerYear)
      if (ctx.portfolioValue && ctx.portfolioValue > 0) {
        const amount = (ctx.portfolioValue * v) / 100
        return `Tu ${fmt(v)}% equivale a unos ${money(amount, ctx.currency)} sobre el valor actual. En 1 de cada 20 ${bars} la pérdida superó esa cifra.`
      }
      return `Tu ${fmt(v)}% es la pérdida que no se superó en 19 de cada 20 ${bars}. Mira también el CVaR: este número no dice qué tan mal fue el vigésimo.`
    },
  },
  ...
  cvar: {
    ...
    definition:
      'El promedio de las pérdidas en los periodos que sí superan el VaR. Si el VaR dice a qué distancia está el borde del acantilado, el CVaR dice qué tan honda es la caída.',
    source: 'Rockafellar y Uryasev (2000), Journal of Risk',
    read: (v, ctx) => {
      const { bars } = cadence(ctx.periodsPerYear)
      if (ctx.portfolioValue && ctx.portfolioValue > 0) {
        const amount = (ctx.portfolioValue * v) / 100
        return `Tu ${fmt(v)}% equivale a unos ${money(amount, ctx.currency)}: es lo que en promedio se perdió en los peores ${bars}, y es la cifra con la que conviene planear.`
      }
      return `Tu ${fmt(v)}% es la pérdida promedio en los peores ${bars}. Siempre es igual o mayor que el VaR, por definición.`
    },
  },
  ...
  maxDrawdown: {
    ...
    definition:
      'La peor caída desde un máximo hasta el fondo siguiente. Mide lo que un inversionista habría tenido que aguantar sin vender.',
    source: null,
    read: (v) => {
      ...
      return `Tu ${fmt(Math.abs(v))}% necesita una ganancia de ${fmt(recovery)}% para volver al punto de partida, no de ${fmt(Math.abs(v))}%: la subida trabaja sobre el saldo más pequeño que dejó la caída.`
    },
  },
  ...
  trackingError: {
    name: 'Tracking error',
    unit: 'percent',
    formula: () => 'TE = desviación estándar(Rp - Rb) x raíz(periodos por año)',
    definition:
      'Qué tanto se separa tu portafolio del índice en un año típico. No es un error que estés cometiendo: un tracking error pequeño solo significa que te pareces al índice.',
    source: null,
    read: (v, ctx) => {
      const bench = ctx.benchmarkName ?? 'el benchmark'
      return `Tu ${fmt(v)}% significa que, en un año típico, tu rendimiento queda a más o menos ${fmt(v)} puntos del de ${bench}. Mayor no es peor: es cuánto te separas, para bien o para mal.`
    },
  },
  ...
  informationRatio: {
    ...
    definition:
      'Si el tracking error mide cuánto te alejas del índice, el information ratio responde si alejarte valió la pena. Aquí se usa el alpha en el numerador; otras fuentes usan el rendimiento activo (Rp - Rb).',
    source: null,
    read: (v) =>
      v > 0
        ? `Tu ${fmt(v)} significa que cada punto de separación del índice te dio ${fmt(v)} puntos de alpha.`
        : v < 0
          ? `Tu ${fmt(v)} significa que separarte del índice te costó: cada punto de separación restó ${fmt(Math.abs(v))} puntos de alpha.`
          : `Tu ${fmt(v)} significa que separarte del índice no te dio ni te quitó alpha.`,
  },

  hhi: {
    name: 'HHI (concentración)',
    unit: 'hhi',
    formula: () => 'HHI = suma de los pesos al cuadrado',
    definition:
      'Qué tan concentrado está el portafolio por peso. Vale 1 si todo está en una sola posición y 1/N si está repartido en partes iguales entre N.',
    source: null,
    read: (v, ctx) => {
      const equivalent = v > 0 ? 1 / v : 0
      const reading = `Tu ${fmt(v, 3)} significa que tu dinero se concentra como si tuvieras ${fmt(equivalent, 1)} posiciones del mismo tamaño.`
      ...
      return `${reading} Tienes ${ctx.holdings} posiciones, así que por peso tu cartera equivale al ${fmt((equivalent / ctx.holdings) * 100, 0)}% de ellas.`
    },
  },
  ...
  effectiveBets: {
    ...
    definition:
      'Cuántas fuentes de riesgo realmente distintas tienes. Diez posiciones que se mueven juntas son una sola apuesta repetida diez veces, y el HHI no lo detecta porque solo mira pesos.',
    ...
    read: (v, ctx) => {
      ...
      return v < 2
        ? `${lead}: en la práctica, casi todo tu riesgo viene de una sola fuente.`
        : `${lead}. La diferencia entre las dos cifras es cuánto se mueven juntos tus activos.`
    },
  },
  ...
  xirr: {
    ...
    definition:
      'Tu rendimiento real considerando cuándo y cuánto aportaste. Si metiste más dinero justo antes de una buena racha, este número lo recoge; el momento de cada aportación importa aquí.',
    source: null,
    read: (v, ctx) => {
      const base = `Tu ${fmt(v)}% anual es lo que efectivamente rindió TU dinero, dado el momento en que lo pusiste. Compáralo con el TWR: si son distintos, la diferencia es el efecto del timing de tus aportaciones.`
      ...
      return (
        ...
        `supone que lo ocurrido en esos ${Math.round(age)} días se repite hasta completar el año. Tómalo como velocidad, no como resultado.`
      )
    },
  },
